...
import { CatalogCategoryAction } from '@/bussiness/actions'
import { useQuery, useMutation, useQueryClient } from '@tanstack/vue-query'
import { computed, ref, watch } from 'vue'
import type { CatalogCategory } from '@/types/catalog-category.type.ts'
import EditCatalogCategoryModal from './EditCatalogCategoryModal.vue'
import CreateCatalogCategoryModal from './CreateCatalogCategoryModal.vue'
...
const selectedItem = ref<CatalogCategory>({} as CatalogCategory)
...
const abrirModal = (item: CatalogCategory) => {
...
const hiddenBtnAdd = computed(() => categories.value?.length === 10)

// 1. Filtrar categorías según el buscador
const categoriesFiltradas = computed(() => {
  if (!categories.value) return []
  if (!searchQuery.value.trim()) return categories.value

  const query = searchQuery.value.toLowerCase()
  return categories.value.filter((cat) => (cat.text ?? '').toLowerCase().includes(query))
})

// 2. Total de registros basados en el filtro actual
const totalRegistros = computed(() => categoriesFiltradas.value.length)
...
// 4. Recorte de categorías para la página actual
const categoriesPaginadas = computed(() => {
  const inicio = (paginaActual.value - 1) * porPagina.value
  const fin = inicio + porPagina.value
  return categoriesFiltradas.value.slice(inicio, fin)
})
...
// Mutación para eliminar la categoría
const { mutate } = useMutation({
  mutationFn: (id: CatalogCategory['id']) => CatalogCategoryAction.remove(id),
  onSuccess: (data) => {
    toast.success(data?.message || 'Categoría eliminada correctamente')
    queryClient.invalidateQueries({ queryKey: ['catalog-categories'] })
    if (categoriesPaginadas.value.length === 1 && paginaActual.value > 1) {
      paginaActual.value--
    }
  },
  onError: (error) => {
...
const confirmarEliminacion = (category: CatalogCategory) => {
...
<template>
  ...
  >
    ...
    >
      <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          Administra las categorías de prendas que se muestran en los filtros de la landing page.
        </p>
      </div>
    ...
    <div class="overflow-x-auto relative">
      <!-- 🔹 Overlay de carga usando también AppSpinner -->
      <div v-if="isFetching && categories" class="absolute inset-0 bg-slate-900/10 dark:bg-slate-900/30 backdrop-blur-[0.5px] z-10 flex items-start justify-center pt-12 pointer-events-none">
        <div class="bg-white dark:bg-slate-800 px-3.5 py-2 rounded-full shadow-md border border-slate-200 dark:border-slate-700">
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          ...
          >
            ...
            <th class="px-6 py-3.5">Predeterminado</th>
            <th class="px-6 py-3.5 text-right">Acciones</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800 text-sm">
          <tr v-if="categoriesPaginadas.length === 0">
            <td colspan="4" class="px-6 py-8 text-center text-slate-400 text-xs">
              No se encontraron categorías que coincidan "{{ searchQuery }}"
            </td>
          </tr>

          <tr
            v-for="(category, index) in categoriesPaginadas"
            :key="index"
          ...
          >
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span v-if="category.isDefault" class="text-emerald-600 dark:text-emerald-400">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right space-x-1">
              ...
              <button
                ...
                :class="{ 'invisible pointer-events-none': category.isDefault || category.id===1 }"
  ...
  <!-- Modal para la edición de la categoría -->
  <EditCatalogCategoryModal
    :isOpen="isModalOpen"
    :id="selectedItem.id"
  ...
  <!-- Modal de creación -->
  <CreateCatalogCategoryModal :isOpen="isCreateModalOpen" @close="isCreateModalOpen = false" />
</template>
